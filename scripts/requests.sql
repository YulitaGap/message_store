...
where author.id = 31 and orders.principal_id = 1 and 
((access_history.date between d1 and d2) or 
 (orders.date between d1 and d2) or 
 (discount.sale_to between d1 and d2) 
 or (allowed_access between d1 and d2));
-- Request #10
select orders.principal_id, style.name, count(orders.id) from orders
inner join posts on orders.post_id = posts.id
inner join style on posts.style_id = style.id
inner join discount on style.id = discount.style_id
where principal_id = 1 and discount = 0.9 and orders.date between d1 and d2
group by style.name, orders.principal_id;
-- Request #12
select max(social_network.name) as s, style.name, count(*) / count(distinct style.name) as counter from social_network
...
where author.id = 33 and posts.date between '2000-01-01' and '2020-08-12'
group by style.name
order by s;
